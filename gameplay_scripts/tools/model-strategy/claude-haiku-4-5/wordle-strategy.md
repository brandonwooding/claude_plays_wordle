...
- The second guess depends on what was learned from the first:
  ...
  - **If guess 1 revealed 1 correct (green) + 1 or more present (yellow) letters**: honour the green (keep it in its confirmed position), re-place each yellow letter in a new valid slot (not the position it was marked yellow in), and fill the remaining slots with fresh untested letters. Do not retreat to pure exploration and risk ignoring the constraints you already have. Example: after SLATE gives L🟩 (pos 2) and A🟡 (pos 3), ALONG is ideal — it locks L at pos 2, moves A to pos 1, and tests O, N, G as new letters. This resolves both known signals while still covering fresh ground
